...
var canvas = $("canvas");
var gridSize = 16;
function ColorVector(value, index, limit, increment) {
...
$("HSLRainbowBtn").addEventListener("click", function () {
    colorMode = "hsl";
});
$("RGBRainbowBtn").addEventListener("click", function () {
    colorMode = "rgb";
});
$("monoBtn").addEventListener("click", function () {
    colorMode = "mono";
});
$("rainbowBias").addEventListener("change", function () {
...
function stepWrappingVector(vector, range) {
    if (range === void 0) { range = 360; }
...
function colorZigZag() {
    for (var i = 0; i < gridSize; i++) {
        var row = Array.from(canvas.querySelectorAll(".row" + i));
        if (i % 2 === 1) {
            row.reverse();
        }
...
function clearGrid(color) {
    var grid = canvas.querySelectorAll(".cell");
...
generateGrid(16);
var colorMode = "mono";
var rainbowBias = false;
var hueVector = new ColorVector(342);
var satVector = new ColorVector(94);
var lightVector = new ColorVector(10);
var redVector = new ColorVector(49);
var blueVector = new ColorVector(2);
var greenVector = new ColorVector(16);
var canvasColor = "hsl(342, 94%, 10%)";
var monoColor = "hsl(81, 89%, 29%)";
var background = "hsl(210, 100%, 7%)";
